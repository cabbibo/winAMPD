...
//float tri(in float x){return (abs(frac(x)-.5))*2;}
...
float triangularNoise( float3 p ){
   ...
   p +=  tri3(p.xyz * .3 + _Time.x *  _NoiseSpeed) *2.6;
   totalFog += triAdd(p.yxz * .3) * .2;
    
   p +=  tri3(p.xyz * .8 + 121+ _Time.x * 2*  _NoiseSpeed ) * 1;
   totalFog += triAdd(p.yxz * 1) * .2;
    
   p +=  tri3(p.xyz * 1.8 + 121+ _Time.x * 3*  _NoiseSpeed ) * 1;
...
float t3D( float3 pos ){
 ...
 // fPos += 100 * mul(unity_ObjectToWorld,float4(0,0,0,1)).xyz;
  return triangularNoise(fPos ) * 1 + triangularNoise(fPos * 3) * .5 + triangularNoise(fPos * 8) * .3;// (.5*noise( fPos) + .3*noise( fPos * 6  )+ .2*noise( fPos * 18  )) * 2 - 1;
}
...
float totalSmoke = 0;
  ...
  float noiseDensity = saturate(length(smoke) - _NoiseSubtractor);
    ...
    col = .99*col +hsv(noiseDensity * .002 + _Time.x,1,noiseDensity) * .04;//lerp( lerp(_BaseColor,_CenterOrbColor , saturate(centerOrbDensity)), _NoiseColor , saturate(noiseDensity));// saturate(dot(v.lightDir , nor)) * .1 *c;//hsv(c,.4, dT3D(p*3,float3(0,-1,0))) * c;//hsv(c * .8 + .3,1,1)*c;;// hsv(smoke,1,1) * saturate(smoke);
